...
/** Pick a random active key from pool. Returns {id, key} or null. */
function _pickPoolKey() {
  if (_aiKeyPool.length === 0) return null;
  return _aiKeyPool[Math.floor(Math.random() * _aiKeyPool.length)];
}
...
async function sendToAI(userMessage, retryCount = 0) {
  ...
  if (poolKeyObj) apiKey = poolKeyObj.key;
  ...
  // Show typing indicator only on first attempt
  ...
  try {
    ...
    // Auto-failover logic for Pool Keys
    const isExhaustedOrInvalid = errMsg.includes('RESOURCE_EXHAUSTED') || errMsg.includes('API_KEY_INVALID') || errMsg.includes('429') || errMsg.includes('quota');

    if (isExhaustedOrInvalid && poolKeyObj && retryCount < 3) {
      console.warn(`[AI Pool] Key ${poolKeyObj.id} failed (${errMsg}). Expiring and retrying...`);
      // Fire & forget expiration
    ...
    if (isExhaustedOrInvalid) {
      finalErrMsg = '❌ Hệ thống AI hiện đang quá tải (Hết quota các key dự phòng). Vui lòng báo lại với quản trị viên hoặc quay lại sau nhé!';
    }
...
function initAIChatbot() {
  ...
  function openPanel() {
    ...
    showWelcomeMessage();
    updateAIKeyStatus(!!(userGeminiKey || GEMINI_FALLBACK_KEY));
...
// We already patched streak above, so just load key when user logs in
const _origOnAuth = window._onAuthSuccess;
// Hook: call loadGeminiKey when user authenticated
(function patchAuthForAI() {
  const originalChangeMainView = window.changeMainView;
  // Load key when profile opens
  const origLoadProfile_AI = window.loadProfile || (() => {});
})();

// Hook into playLecture to update context
const _origPlayLecture = window.playLecture;

// ---- Init ----
initAIChatbot();

// Load AI key when user is already logged in
if (currentUser) {
  loadAIKeyPool();
}
