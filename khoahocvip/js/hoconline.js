...
function renderCourseGrid() {
  ...
  const generateCardHtml = (course, idx, isGrid) => {
    ...
    const isUnlocked = !currentUser || isAdmin || unlockedCourses.includes(course.id) || hasFreeTrial;
...
async function loadNotifications() {
  ...
  try {
    // Collect from news + qa answers (no extra collection needed)
    const [newsSnap, qaSnap] = await Promise.all([
...
async function activateKey(keyCode) {
  ...
  try {
    ...
    // Merge courseIds into user's unlockedCourses
    const existingUnlocked = userProgress.unlockedCourses || [];
    const newUnlocked = [...new Set([...existingUnlocked, ...(keyData.courseIds || [])])];
    userProgress.unlockedCourses = newUnlocked;

    await setDoc(doc(db, 'progress', currentUser.email), { unlockedCourses: newUnlocked }, { merge: true });
    ...
    const courseNames = (keyData.courseIds || []).map(cId => {
    ...
    showToast('Lỗi kích hoạt mã. Vui lòng thử lại!', true);
  } finally {
...
// Update loadProgress to also read unlockedCourses
const _origLoadProgress = loadProgress;
window._origLoadProgress = _origLoadProgress;

// Patch the existing loadProgress function reference
// (hoconline.js already has it — we extend userProgress on load)
const _hoconlineLoadProgressPatch = async () => {
  if (!currentUser) return;
  try {
    const snap = await getDoc(doc(db, 'progress', currentUser.email));
    if (snap.exists()) {
...
// Override loadProgress to also load unlockedCourses
// We do this safely by calling the patch after the existing call
setTimeout(() => {
  if (currentUser) _hoconlineLoadProgressPatch();
}, 2500);
