...
async function sendToAI(userMessage, retryCount = 0) {
  ...
  if (poolKeyObj === 'COOLDOWN') {
    ...
    appendAIMessage('ai', '⚠️ AI đang bị Rate Limit. Đợi 1 phút rồi thử lại nhé!');
    return;
  }
  ...
  if (!apiKey) {
    removeTypingIndicator(document.getElementById('ai-typing-row'));
    appendAIMessage('ai', '⚠️ Hệ thống hiện đang hết Key phục vụ. Xin vui lòng báo lại với Admin!');
    return;
  }
  ...
  // Sử dụng cấu trúc systemInstruction chuẩn của Google
  const requestBody = {
    systemInstruction: {
  ...
  try {
    ...
    console.error(`Gemini API error (Lần thử ${retryCount + 1}):`, err);

    const isInvalid = errMsg.includes('API_KEY_INVALID') || errMsg.includes('API key not valid');
    const isDailyQuota = errMsg.includes('per day') || errMsg.includes('quota');
    const isRateLimit = errMsg.includes('per minute') || errMsg.includes('429');

    if ((isInvalid || isDailyQuota) && poolKeyObj && retryCount < 3) {
      _expirePoolKey(poolKeyObj.id);
      return sendToAI(userMessage, retryCount + 1);
    } else if (isRateLimit && poolKeyObj && retryCount < 3) {
    ...
    // HIỂN THỊ LỖI GỐC RA MÀN HÌNH ĐỂ BẮT BỆNH
    appendAIMessage('ai', `🚨 Lỗi từ Google: ${errMsg}`);
  }
}
