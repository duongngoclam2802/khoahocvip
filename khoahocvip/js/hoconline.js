...
async function activateKey(keyCode) {
  ...
  try {
    ...
    // Use arrayUnion directly to append the courses securely and avoid race conditions
    batch.set(progressRef, { 
      unlockedCourses: arrayUnion(...(keyData.courseIds || [])) 
    }, { merge: true });
    ...
    try {
      await addDoc(collection(db, 'notifications'), {
        ...
        body: `Bạn đã nhận được các khóa học: ${courseNames.join(', ')}`,
        createdAt: new Date().toISOString(),
        icon: 'gift',
        color: 'text-orange-500',
        bg: 'bg-orange-50 dark:bg-orange-900/20',
