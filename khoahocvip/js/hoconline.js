...
async function sendToAI(userMessage, retryCount = 0) {
  ...
  try {
    ...
    const isExhausted = errMsg.includes('RESOURCE_EXHAUSTED') || errMsg.includes('429') || errMsg.includes('quota');

    if (isInvalid && poolKeyObj && retryCount < 3) {
    ...
    } else if (isExhausted && poolKeyObj && retryCount < 3) {
      console.warn(`[AI Pool] Key ${poolKeyObj.id} exhausted/rate-limited. Cooling down for 1 min...`);
