...
window.copyKey = (key) => {
  navigator.clipboard.writeText(key).then(() => showToast(`Đã copy: ${key}`));
};
...
if (btnConfirmManualActivate) {
  btnConfirmManualActivate.addEventListener('click', async () => {
    ...
    try {
      // Read current progress doc (to merge, not overwrite)
      const progressRef = doc(db, 'progress', email);
      const progressSnap = await getDoc(progressRef);
      const existing = progressSnap.exists() ? (progressSnap.data().unlockedCourses || []) : [];
      const merged = [...new Set([...existing, ...courseIds])];

      await setDoc(progressRef, { unlockedCourses: merged }, { merge: true });
