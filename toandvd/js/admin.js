...
onAuthStateChanged(auth, async (user) => {
  ...
  if (user) {
    if (user.email !== 'duongngoclam28022008@gmail.com') {
...
btnLogin.addEventListener('click', async () => { await signInWithPopup(auth, googleProvider); });
...
function renderTopicsEditor() {
  ...
  container.innerHTML = '';
  editingCourse.topics.forEach((topic, tIndex) => {
    const tDiv = document.createElement('div');
    tDiv.className = 'p-4 bg-white dark:bg-slate-800 rounded-xl border border-theme shadow-sm';
    tDiv.innerHTML = `
      <div class="flex gap-2 mb-3">
        <input type="text" class="input-glass !py-1 flex-1 font-bold" value="${topic.title}" onchange="editingCourse.topics[${tIndex}].title=this.value">
        <button class="btn-secondary !py-1 !px-2 text-xs" onclick="addLecture(${tIndex})">Thêm bài</button>
        <button class="btn-secondary text-red-500 !py-1 !px-2" onclick="editingCourse.topics.splice(${tIndex}, 1); renderTopicsEditor()"><i data-lucide="trash" class="w-4 h-4"></i></button>
      </div>
      <div class="space-y-3 pl-4 border-l-2 border-theme ml-2" id="lectures-${tIndex}"></div>
    `;
    container.appendChild(tDiv);
    const lContainer = tDiv.querySelector(`#lectures-${tIndex}`);
    if(topic.lectures) topic.lectures.forEach((lec, lIndex) => {
      const lDiv = document.createElement('div');
      lDiv.className = 'bg-slate-50 dark:bg-slate-900 p-3 rounded-xl border border-theme';
      lDiv.innerHTML = `
        <div class="flex gap-2 mb-2">
          <input type="text" class="input-glass !py-1 flex-1 text-sm font-semibold" value="${lec.title}" placeholder="Tên bài..." onchange="editingCourse.topics[${tIndex}].lectures[${lIndex}].title=this.value">
          <button class="text-red-500" onclick="editingCourse.topics[${tIndex}].lectures.splice(${lIndex},1); renderTopicsEditor()"><i data-lucide="x" class="w-4 h-4"></i></button>
        </div>
        <div class="space-y-2">
          <input type="text" class="input-glass !py-1 text-xs" placeholder="Link Youtube" value="${lec.youtubeLink||''}" onchange="editingCourse.topics[${tIndex}].lectures[${lIndex}].youtubeLink=this.value">
          <textarea class="input-glass !py-1 text-xs" rows="2" placeholder="Mô tả..." onchange="editingCourse.topics[${tIndex}].lectures[${lIndex}].description=this.value">${lec.description||''}</textarea>
          <input type="text" class="input-glass !py-1 text-xs" placeholder="Link File PDF" value="${lec.documentLink||''}" onchange="editingCourse.topics[${tIndex}].lectures[${lIndex}].documentLink=this.value">
          <label class="flex items-center gap-2 text-xs text-main"><input type="checkbox" ${lec.isFreeTrial?'checked':''} onchange="editingCourse.topics[${tIndex}].lectures[${lIndex}].isFreeTrial=this.checked">Học thử</label>
        </div>
      `;
...
window.addLecture = (tIndex) => {
  if(!editingCourse.topics[tIndex].lectures) editingCourse.topics[tIndex].lectures = [];
  editingCourse.topics[tIndex].lectures.push({ id: Math.random().toString(36).substr(2,9), title: "", youtubeLink: "", documentLink: "", description: "", isFreeTrial: false });
  renderTopicsEditor();
}
document.getElementById('btn-add-topic').addEventListener('click', () => {
  editingCourse.topics.push({ id: Math.random().toString(36).substr(2,9), title: "", lectures: [] });
  renderTopicsEditor();
});

document.getElementById('btn-save-course').addEventListener('click', async () => {
  editingCourse.title = document.getElementById('course-name').value;
  editingCourse.thumbnailUrl = document.getElementById('course-thumbnail').value;
  const payload = { title: editingCourse.title, thumbnailUrl: editingCourse.thumbnailUrl, topics: editingCourse.topics, updatedAt: new Date().toISOString() };
  if(editingCourse.id) await updateDoc(doc(db, "courses", editingCourse.id), payload);
  else { payload.createdAt = new Date().toISOString(); await addDoc(collection(db, "courses"), payload); }
    ...
    div.innerHTML = `
      <div class="flex justify-between items-start mb-3">
        <div class="flex items-center gap-2">
          <div class="w-8 h-8 bg-blue-100 text-blue-600 rounded-full flex items-center justify-center font-bold text-xs">${data.userEmail[0].toUpperCase()}</div>
          <div>
            <p class="text-sm font-bold text-main">${data.userEmail}</p>
